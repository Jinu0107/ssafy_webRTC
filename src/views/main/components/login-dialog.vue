    ...
    */
    ...
    const validatorId = (rule, value, callback) => {
      if (value === "") {
        ...
        callback(new Error("필수값이 비어있습니다."));
      } else if (value.length > 16) {
        state.btnDisabled = true;
        callback(new Error("최대 16글자까지 입력 가능합니다."));
      } else {
        loginForm.value.validateField("password", errorMessage => {
          if (errorMessage === "") {
    ...
    const validatorPass = (rule, value, callback) => {
      if (value === "") {
        ...
        loginForm.value.validateField("id", errorMessage => {
          if (errorMessage === "") {
            state.btnDisabled = false;
          } else {
            state.btnDisabled = true;
    ...
    const clickLogin = function() {
      ...
      loginForm.value.validate(async valid => {
        if (valid) {
          ...
          try {
            let model = { id: state.form.id, password: state.form.password };
            const { data } = await store.dispatch("root/requestLogin", model);
